...
**Tech stack** (Path B): Faust for DSP → `faust -lang rust` → nih-plug for plugin shell → CLAP + VST3 fall out. Python for offline lookup-table generation. Linux primary target.
...
1. Toolchain validation: trivial Faust → Rust → nih-plug → CLAP loading on Linux. Stop and verify before proceeding.
2. Port Keller's filter primitives to Faust (most map to `fi.lowpass`, `fi.tf2`, `fi.svf` in stdlib).
3. Port ADAA + GLF table system (Python offline tables + Faust runtime).
4. Port one tube class (`tube_ck`); ABX-verify against Keller's JSFX.
5. Port remaining stages, wire up 5E3, full-amp ABX.
6. T1.2 (cut post-filter, output line level for IR loader).
...
faust -lang rust → Rust source

nih-plug Rust shell
  ├─ Plugin format adapters (CLAP, VST3)
  ├─ GUI, parameter automation
...
| Plugin shell | nih-plug | CLAP + VST3 |
| DSP language | Faust | Block-diagram syntax matches paper figures |
...
| **Rust + nih-plug** | **Recommended for shell** — CLAP+VST3, free, modern, Linux-native |
...
| **Faust → Rust** | **Recommended for DSP** — block-diagram syntax matches problem |

**Final stack**: Faust (DSP) → compile to Rust → nih-plug (plugin shell) → CLAP/VST3, primary target Linux.
...
- Is offline-table-generation really faster than runtime closed-form W evaluation in Faust/Rust on modern CPUs?
...
- Can the negative feedback loop (in Marshall/Twin amps) be wired through Faust's `~` operator across stages without breaking the offline closed-loop trick?
