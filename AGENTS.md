...
`make native-host-test` is the explicit REAPER-dependent CLAP host smoke test.
It uses temporary `CLAP_PATH` discovery and must not be folded into the default
native test path.
